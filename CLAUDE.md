...
- List item extras: `measures[]` ({amount,unit}|{text}) stored as JSON in the `qty` column; `sources[]` ({id,name} of contributing recipes) stored as JSON in `source_recipe_id`. Manual items have both empty
- Grocery auto-populate: `addRecipesToGrocery` (and `addRecipeToGrocery`/`addWeekToGrocery`) appends ingredients; recipe-sourced items dedupe/aggregate against each other and existing recipe-sourced lines by `groceryKey` (name sans prep notes), summing `measures` with matching `normUnit`; manual items are never merged into. `computeDupKeys` flags items present as both manual and recipe-sourced ("dup" badge). Entry points: Recipe detail "Add to grocery", grocery drawer "Add this week's meals"
