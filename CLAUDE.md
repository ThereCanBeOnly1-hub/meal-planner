...
- Remove-by-recipe: `removeRecipeFromGrocery(recipeId)` deletes grocery items only that recipe added; items shared with other recipes keep the item and just drop the recipe from `sources`. Surfaced as a "🛒 Remove from grocery (N)" button on RecipeDetail (shown when `groceryCountForRecipe` > 0).
